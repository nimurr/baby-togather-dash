...
import { Modal, Input, message, Form, Upload, Pagination } from 'antd';
...
const BabyCuseSouthingdetails = () => {
    ...
    // Fetching data based on pagination parameters (currentPage and pageSize)
    const { data, refetch } = useGetAllBabucareQuery({
        type: "Soothing",
        page: currentPage,
        limit: pageSize,
    });
    ...
    // Paginate the filtered cues
    const getPaginatedData = () => {
        return filteredCues.slice(0, pageSize);
    };
    ...
    const handleEditClick = (cue) => {
        ...
        setAudio(null); // Reset the audio file for edit mode
        setIsModalVisible(true);
    };
    ...
    const handleDeleteClick = async (id) => {
        try {
            ...
            if (res?.data?.code === 200) {
                ...
                setBabyCues(babyCues.filter((cue) => cue.id !== id)); // Remove the deleted cue from the list
                setFilteredCues(filteredCues.filter((cue) => cue.id !== id)); // Remove the deleted cue from the filtered list
            }
        } catch (error) {
    ...
    const handleModalSubmit = async () => {
        ...
        // If the audio is provided (new or edited), append it
        if (audio && audio.originFileObj) {
            formData.append('image', audio.originFileObj); // Append the actual audio file
        } else if (currentCue?.audio) {
            formData.append('image', currentCue?.audio); // Append the previous audio if no new file is uploaded
        }
    ...
    const handlePageChange = (page) => {
        ...
        refetch(); // Refetch the data for the selected page
    };
                    ...
                    <Form.Item label="Upload Audio" required>
                        ...
                        >
                            ...
                            <p className="ant-upload-text">
                                {audio ? ' Audio Uploaded' : 'Click or drag audio to this area to upload'}
                            </p>
                        </Upload.Dragger>
                    </Form.Item>
                </Form>
